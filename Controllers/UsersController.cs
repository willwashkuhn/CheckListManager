...
namespace CheckListManager.Controllers
{
    public class UsersController : ApiController
    {
        ...
        public IHttpActionResult DeleteUser(int id)
        {
            ...
            List<CheckList> checkLists = db.CheckLists.Where(l => l.UserId == id).ToList();
            foreach (var cl in checkLists)
            {
                List<CheckListItem> checkListItems = db.CheckListItems.Where(i => i.CheckListId == cl.CheckListId).ToList();
                foreach (var cli in checkListItems)
                {
                    db.CheckListItems.Remove(cli);
                }
                db.CheckLists.Remove(cl);
            }
